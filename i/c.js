var
	...
	Q=function($,win,doc){
		...
		var toRgb = function(c){
			...
			if(/^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/.test(l)){
				if(l.length === 4){
					for(var i=1; i<4; i+=1){
						s += l.slice(i,i+1).concat(l.slice(i,i+1));
					}
					l = s;
				}

				for(var i=1; i<7; i+=2){
					r.push(parseInt("0x"+l.slice(i,i+2)));
				}
				r.push(p);
				return "RGBA(" + r.join(",") + ")";
			}else{
		...
		win.onscroll = function(e){
			...
			var n = $('#nav');
			if (win.scrollY > n.offsetTop){
				$.D.b(body,n);
				n.className = 'nav-f';
			}
			if (win.scrollY <= $.S('header')[0].offsetHeight) {
				$.D.a($.S('header')[0], n);
				n.className = '';
			}
		}
		console.log('ヾ(≧∇≦)〃可能是世界最快博客 v2 @卜卜口<mouto.sinaapp.com> 2014/08/30','\n\n','THEME V2 By Ylune<une.moe> 2016/07/18');
